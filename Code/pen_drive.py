...
class USBRecursiveScanner():
    ...
    def get_usb_mounts(self):
        ''' This function explore the device tree and find the mountpoints of the USB children.'''
        try:
            ...
            for device in data.get('blockdevices', []):
                #If the primary device is a USB drive or removable storage device, check both it and its secondary devices
                if device.get('tran') == 'usb' or device.get('rm') == '1':
                    self._extract_mountpoints(device, all_mounts)
            
            return list(set(all_mounts)) #No duplicates
    ...
    def RemoveLine(self, text):
        ...
        if x:
            try:
                row = "".join(self.row_filtrer_beginning(global_variable.main_filename_application, x.hex())) #check the row
