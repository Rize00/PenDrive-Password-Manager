...
This module is for the pendrive. 
'''
...
import crypting
from utility_class_and_functions import windows_set_hidden_file,windows_set_visible_file
...
class USBRecursiveScanner():
    ...
    def __init__(self):
        ...
        self.hash_registry = global_variable.hash_filename_application
    ...
    def update_file_compact(self,text_name, new_line=None, remove_line=None):
        ...
        try:
            ...
            try:
                #Fix permission denied in windows
                windows_set_visible_file(filepath)
                windows_set_visible_file(self.hash_registry)
                windows_set_visible_file(global_variable.main_filename_backup_application)
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                for i, l in enumerate(lines):
                    f.write(l + "\n") #\n is a good choice
        except Exception as e: print(f"End writiting file: {e}")
        #DO A BACKUP
        try:
            shutil.copy2(filepath, self.path / global_variable.main_filename_backup_application)
        except Exception as e: pass
        #Update the hash
        new_hash = self.CountHash()
        with open(self.hash_registry, "w") as f:
            f.write(new_hash)

        try:
            windows_set_hidden_file(filepath)
            windows_set_hidden_file(self.hash_registry)
            windows_set_hidden_file(global_variable.main_filename_backup_application)
        except Exception as e: pass
    ...
    def CountHash(self):
        ...
        try:
            with open(global_variable.main_filename_application, "rb") as f:
    ...
    def GetHash_registry(self): return self.hash_registry
