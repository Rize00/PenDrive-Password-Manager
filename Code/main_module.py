...
class GUI_option_linux(FloatLayout):
        ...
        def SetKey(self): #SetKey button Command
                ...
                if len(key) == 16 and self.ids.key_input.readonly==False:
                        ...
                        #Show some errors
                        Show_popup(title_=Language._("key error"), text_=Language._("key len error msg"),icon_type_="warning", width_=400,height_=400, f_size=28,
        ...
        def CopyPassword(self, instance):
                if global_variable.KeyString != "": #Check if key string is inserted
                        ...
                        from kivy.core.clipboard import Clipboard
                        Clipboard.copy(" ")
                        Show_popup(title_=Language._("key error"), text_=Language._("key set msg"),icon_type_="warning", width_=400,height_=400, f_size=24,
                                   btn_width_=0.5, btn_height_=0.5)
                ...
                try:
                        if global_variable.current_path and global_variable.KeyString:
                                if first_entry != "" and second_entry != "":
                                        ...
                                        if not founded_row_first_entry:
                                                ...
                                                Show_popup(title_=Language._("form error"), text_=Language._("form error msg username"),
                                                           icon_type_="warning", width_=400,height_=400, f_size=24,
                                                           btn_width_=0.5, btn_height_=0.5) 
                                else:
                                ...
                                if global_variable.current_path == "": #if no path
                                        Show_popup(title_=Language._("error path"), text_=Language._("error path msg"),icon_type_="warning", width_=400,height_=400, f_size=30,
                                                   btn_width_=0.5, btn_height_=0.5)
                                if global_variable.KeyString == "": #if no key
                                        Show_popup(title_=Language._("key error"), text_=Language._("key error msg"),icon_type_="warning", width_=400,height_=400, f_size=30,
                                                   btn_width_=0.5, btn_height_=0.5) 
                except Exception as e: pass
...
def Show_popup(title_, text_, icon_type_="info", hex_bg="#ffffff",width_=400, height_=250, f_size=14, btn_width_=0, btn_height_=0):
    content = CustomPopup(message_text=text_, icon_type=icon_type_, bg_hex=hex_bg, custom_font_size=f"{f_size}sp", btn_width = btn_width_, btn_height=btn_height_)
    
    pop = Popup(title=title_, content=content, size_hint=(None, None), size=(width_, height_), auto_dismiss=False if icon_type_ == "error usb" else True)
...
class Interface(MDApp): #Main application window
        ...
        def create_menu_bar(self):
                ...
                icon = get_resource_path("com.rize.pendrivepasswordmanager.png")
                action_prev = ActionPrevious(title="", with_previous=False, height='80dp', app_icon=icon,
                ...
                if not self.subvoice:
                        btn_nopath = ActionButton(text="No Path", size_hint_x=2, font_size=30, width='20dp',height='22dp', color=utils.get_color_from_hex("#FFD700"))
                        btn_nopath.bind(on_press=lambda x: self.USBListening())
                        action_view.add_widget(btn_nopath)
                else:
                        dropdown_menu = ActionGroup(text='Path List', mode='spinner', size_hint_x=None, width='400dp',
                                                    dropdown_width='450dp', font_size=30,height='22dp',
                                                    color=global_variable.color_menu_voice)

                        for i in range(len(self.subvoice)): #Temporary group
                                item = ActionButton(text=str(self.subvoice[i]),height='22dp', size_hint_y=None, size_hint_x=None, font_size=30,
                ...
                btn_help = ActionButton(text=Language._('help box'), size_hint_x=2, font_size=30, width=20,height='22dp', color=global_variable.color_menu_voice)
                btn_help.bind(on_press=lambda x: self.HowToUse())
                action_view.add_widget(btn_help)

                btn_about = ActionButton(text=Language._('about'), size_hint_x=2, font_size=30, width=20,height='22dp', color=global_variable.color_menu_voice)
                btn_about.bind(on_press=lambda x: self.About())
                action_view.add_widget(btn_about)

                btn_donation = ActionButton(text=Language._('donation'), size_hint_x=2, font_size=30, width=20,height='22dp', color=global_variable.color_menu_voice)
        ...
        def About(self):
                Show_popup(title_=Language._("help box"), text_=Language._("help msg"),icon_type_="account", width_=800,height_=800, f_size=24,
                           btn_width_=0.5, btn_height_=0.5)
        ...
        def USBListening(self):
                import subprocess
                import sys
                subprocess.Popen([sys.executable] + sys.argv) #Create a new process
                os._exit(0) #exit from this one
        ...
        def ShowErrorUSB(self, dt):
                try:
                        ...
                        os.chdir(main_dir)
                        Show_popup(title_=Language._("error no pendrive"), text_=Language._("no pendrive"),icon_type_="usb error", width_=670,height_=350, f_size=36,
                                   btn_width_=0.5, btn_height_=0.5)
                except Exception as e: pass
